...
public:
	...
	void Apply(const Resources& resources, u8 render_alpha, bool modulate) const;
	const char *getName() const { return header->name; }

	const Material::Header *GetHeader() const { return header; }

	// used for the widescreen textures in the banner frame, channel grid,  and
	// to set the digits for the clock.  this is an index for textures listed in the brlyt, not the brlan
	void SetTextureIndex( u8 whichTextureMap, u8 whichTextureToUse )
	{
		if( header && whichTextureMap < flags->texture_map )
		{
			texture_maps[ whichTextureMap ].tex_index = whichTextureToUse;
		}
	}
