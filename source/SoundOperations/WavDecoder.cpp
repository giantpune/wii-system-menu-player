...
void WavDecoder::OpenFile()
{
	...
	if (Header.magicRIFF != MAKE_FOURCC('R','I','F','F'))
	{
		CloseFile();
		return;
	}
	else if(Header.magicWAVE != MAKE_FOURCC('W','A','V','E'))
	{
		CloseFile();
		return;
	}
	else if(FmtChunk.magicFMT != MAKE_FOURCC('f','m','t',' '))
	{
		CloseFile();
		return;
	}
	...
	while(DataChunk.magicDATA != MAKE_FOURCC('d','a','t','a'))
	{
