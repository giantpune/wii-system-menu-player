...
void Animator::LoadKeyFrames(const u8 *file, u8 tag_count, u32 offset )
{
	...
	for(u32 tag = 0; tag < tag_count; tag++)
	{
		...
		for(u32 frame = 0; frame < frame_count; frame++)
		{
			...
			switch (keyFrame->data_type)
			{
				// step key frame
			case 0x01:
				keys.step_keys[frame_type].Load((const u8 *) (keyFrame+1), keyFrame->key_count);
				break;

				// hermite key frame
			case 0x02:
				keys.hermite_keys[frame_type].Load((const u8 *) (keyFrame+1), keyFrame->key_count);
				break;
...
{
	keys.step_keys.clear();
	keys.hermite_keys.clear();
}

void Animator::SetFrame(FrameNumber frame_number)
{
	std::map<KeyType, HermiteKeyHandler>::iterator itr;
	for(itr = keys.hermite_keys.begin(); itr != keys.hermite_keys.end(); itr++)
	{
	...
	for(itr2 = keys.step_keys.begin(); itr2 != keys.step_keys.end(); itr2++)
	{
