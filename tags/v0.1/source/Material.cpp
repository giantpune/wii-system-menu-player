...
void Material::Load(Material::Header *file)
{
	...
	flags->texture_map = MIN(MAX_TEX_MAP, flags->texture_map );
	flags->texture_srt = MIN(MAX_TEX_SRT, flags->texture_srt );
	flags->texture_coord_gen = MIN(MAX_TEX_GEN, flags->texture_coord_gen );
	flags->ind_srt = MIN(MAX_IND_TEX_SRT, flags->ind_srt );
	flags->ind_stage = MIN(MAX_IND_STAGES, flags->ind_stage );
	flags->tev_stages = MIN(MAX_TEV_STAGES, flags->tev_stages );
...
inline void Material::ApplyTextures(const Resources& resources) const
{
	u8 tlut_name = 0;
	...
	for(u32 i = 0; i < flags->texture_map; ++i)
	{
		...
		if( forcedTexture )
		{
			forcedTexture->Apply(tlut_name, i, tr.wrap_s, tr.wrap_t);
		}
		else if(palette_texture[i] == DEFAULT_PALETTE)
		{
		...
		else
		{
			...
			if( !animationLink
				|| !animationLink->GetAnimation()
				|| palette_texture[ i ] >= animationLink->GetAnimation()->paletteNames.size() )
			{
				gprintf( "palette index is out of range %i\n", palette_texture[i]);
				return;
			}

			// TODO: change the way this works so we aren't comparing strings 60 frames a second for every pane that using this material
			// ie, in the space invader wiiware banner, it draws like 50 of the same image and we end up comparing the names a shitload when it isnt
			// necessary
			for(u32 n = 0; n < resources.textures.size(); n++)
			{
				if( resources.textures[n]->getName() == animationLink->GetAnimation()->paletteNames[ palette_texture[ i ] ] )
				{
					//gprintf( "applying texture[%u] %s\n",palette_texture[i], resources.textures[n]->getName().c_str() );
