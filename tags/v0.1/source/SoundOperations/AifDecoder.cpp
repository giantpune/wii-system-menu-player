...
void AifDecoder::OpenFile()
{
	...
	if (Header.magicRIFF != MAKE_FOURCC('F','O','R','M'))
	{
		CloseFile();
		return;
	}
	else if(Header.magicWAVE != MAKE_FOURCC('A','I','F','F'))
	{
		CloseFile();
		return;
	}
	...
	if(CommHdr.fccCOMM != MAKE_FOURCC('C','O','M','M'))
	{
		CloseFile();
		return;
	}
	...
	while(ret > 0 && chunkHdr.magicDATA != MAKE_FOURCC('S','S','N','D'));
	...
	if(SSndChunk.fccSSND != MAKE_FOURCC('S','S','N','D'))
	{
		CloseFile();
		return;
	}
